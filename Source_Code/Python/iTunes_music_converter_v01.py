...
#-
	import urllib2;
	escape = lambda input: urllib2.unquote(input).encode('utf-8');                # Escape any HTML style encoding and convert to UTF-8
	meta = [];                                                                    # Initialize list
# 	if ('Name' in info): 
# 		meta.extend(['-metadata', 'title='        + escape(info['Name'])]);         # Append the track name to the command
# 	if ('Artist' in info):  
# 		meta.extend(['-metadata', 'artist='       + escape(info['Artist'])]);       # Add the artist to the command
# 	if ('Album Artist' in info):
# 		meta.extend(['-metadata', 'album_artist=' + escape(info['Album Artist'])]); # Add the album artist to the command
# 	if ('Album' in info):
# 		meta.extend(['-metadata', 'album='        + escape(info['Album'])]);        # Add the album to the command
# 	if ('Composer' in info):
# 		meta.extend(['-metadata', 'composer='     + escape(info['Composer'])]);     # Add the composer to the command     
# 	if ('Genre' in info):
# 		meta.extend(['-metadata', 'genre='        + escape(info['Genre'])]);        # Add the genre to the command
# 	if ('Year' in info):
...
# 	if ('Name' in info): meta['title']  = escape(info['Name']);                                      # Append the track name to the dictionary
# 	if ('Artist' in info): 
# 		meta['artist'] = escape(info['Artist']);                                    # Add the artist to the dictionary
# 	if ('Album Artist' in info): 
# 		meta['albumartist'] = escape(info['Album Artist']);                         # Add the album artist to the dictionary
# 	if ('Album' in info):
# 		meta['album'] = escape(info['Album']);                                      # Add the album to the dictionary
# 	if ('Composer' in info):
# 		meta['composer'] = escape(info['Composer']);                                # Add the composer to the command     
# 	if ('Genre' in info):
# 		meta['genre'] = escape(info['Genre']);                                      # Add the genre to the dictionary
# 	if ('Year' in info):
