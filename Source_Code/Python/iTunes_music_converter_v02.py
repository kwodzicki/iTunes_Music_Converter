...
def iTunes_music_converter(dest_dir=None, \
  track_id = None, \
  bit_rate = None, \
  codec    = None, \
  verbose  = False, \
...
#   dryrun   : Set to perform a dry run and show only those files that would be
#              copied/converted
# Author and History:
#   Kyle R. Wodzicki     Created 24 May 2016
#
...
#     Modified 05 Mar. 2017 by Kyle R. Wodzicki
#       Added the dryrun keyword
#-
	import os, time, sys;
...
# 			cmd = [ffmpeg, '-y', '-threads', '1', '-i', source];    # Set up the command to run with much of output suppressed and running on only one thread
		...
		if os.path.isfile(destination): 
			if verbose: print '    File EXISTS on receiver!';                         # Some verbose output
			continue;                                                                 # If the destination file already exists, continue past it
		cover_code = 1;																															# Set cover code to 1 as default; NO cover art
		...
		if os.path.isfile(cover):
			if os.stat(cover).st_size > 0: cover_code = 0;                            # Set cover code to zero if cover art file exists
		...
		nTry = 0;																																		# Set number of tries
		return_code = 0 if dryrun else 1;																						# Set default return code value
		while (return_code != 0) and (nTry < 3):
			return_code = call(cmd);                                                  # Run the command
			nTry+=1;
	
		if (return_code == 0) and (codec == 'flac') and not mpeg_test and not dryrun:
...
if __name__ == "__main__":
	...
	parser.add_argument("-n", "--dryrun", action='store_true', \
	  help="perform a dry run; do not convert")
	...
	return_code = iTunes_music_converter(dest_dir = args.dir, \
	                                     ...
	                                     dryrun   = args.dryrun); # Call the function to write the tags